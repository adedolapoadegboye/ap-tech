...
const ProductPageIphone = (props) => {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const { all, series15, series14, series13, series12, series11, seriesx, se } =
    props;
  console.log(
    all,
    series15,
  ...
  }, [all, series15, series14, series13, series12, series11, seriesx, se]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        let apiLink = `http://35.160.225.128:4000/iphone/${api}`;
        console.log(apiLink);
